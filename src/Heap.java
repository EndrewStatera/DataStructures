...
public class Heap {
    ...
    private void sift_down ( int pos ) {
        int left = left(pos);
        int right = right(pos);
        int indiceMaisAlto = pos;
        if(v[left] > v[indiceMaisAlto]){
            indiceMaisAlto = left;
        }
        if(v[right] > v[indiceMaisAlto]){
            indiceMaisAlto = right;
        }
        if(indiceMaisAlto != pos){
            int aux = v[indiceMaisAlto];
            v[indiceMaisAlto] = v[pos];
            v[pos] = aux;
            sift_down(indiceMaisAlto);
        }
    }
